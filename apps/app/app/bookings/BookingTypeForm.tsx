...
export interface BookingTypeValues {
  slug: string;
  title: string;
...
type Action = (prev: BookingFormState, form: FormData) => Promise<BookingFormState>;
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <label className="block">
        ...
        <input name="title" defaultValue={initial?.title ?? ""} required placeholder="1:1 Strategy Call" className={inputCls} />
      </label>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <label className="block">
          ...
          <input name="durationMins" inputMode="numeric" defaultValue={initial?.durationMins ?? ""} placeholder="30" className={inputCls} />
        </label>
        <label className="block">
      ...
      <label className="block">
        <span className="text-sm font-medium text-zinc-700">Meeting link (Zoom / Meet)</span>
        <input name="meetingUrl" defaultValue={initial?.meetingUrl ?? ""} placeholder="https://zoom.us/j/… or https://meet.google.com/…" className={inputCls} />
        <span className="mt-1 block text-xs text-muted">Revealed only to people who book + pay.</span>
      </label>
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <Link href="/bookings" className="text-sm text-muted underline">
          {isEdit ? "Back to 1-on-1s" : "Cancel"}
        </Link>
      </div>
    </form>
  );
