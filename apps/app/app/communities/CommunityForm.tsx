...
type Action = (prev: CommunityFormState, form: FormData) => Promise<CommunityFormState>;
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <label className="block">
        ...
        <input name="title" defaultValue={initial?.title ?? ""} required placeholder="The Inner Circle" className={inputCls} />
      </label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <label className="block">
          ...
          <input
            ...
            className={inputCls}
          />
          <span className="mt-1 block text-xs text-muted">Leave blank or 0 for a free community.</span>
        </label>
        <label className="block">
        ...
        <label className="block">
          <span className="text-sm font-medium text-zinc-700">Members link</span>
          <input name="accessUrl" defaultValue={initial?.accessUrl ?? ""} placeholder="https://t.me/… (Telegram/Discord/…)" className={inputCls} />
          <span className="mt-1 block text-xs text-muted">Revealed only to members in the members area.</span>
        </label>
      </div>
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <Link href="/communities" className="text-sm text-muted underline">
          {isEdit ? "Back to communities" : "Cancel"}
        </Link>
      </div>
    </form>
  );
