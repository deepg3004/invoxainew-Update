...
import {
  ...
  startCommunityUpiSession,
  previewCommunityCoupon,
  joinFreeCommunityAction,
} from "./actions";
...
export function CommunityJoinBox({
  community,
  razorpayReady,
  upi,
}: {
  community: { id: string; slug: string; title: string; pricePaise: number };
  ...
  const discount = applied ? Math.min(applied.discountPaise, community.pricePaise) : 0;
  const total = Math.max(0, community.pricePaise - discount);
  ...
  useEffect(() => {
    ...
    (async () => {
      setApplying(true);
      try {
        const res = await previewCommunityCoupon(community.id, c);
  ...
  async function joinFree() {
    setError(null);
    setStatus("starting");
    const res = await joinFreeCommunityAction(community.id);
    if (res.ok) {
      router.push(`/account/community/${res.slug}`);
      return;
    }
    if (res.needLogin) {
      router.push(`/account/login?next=/m/${community.slug}`);
      return;
    }
    setError(res.error);
    setStatus("idle");
  ...
  async function applyPromo() {
    ...
    setCouponMsg(null);
    try {
      const res = await previewCommunityCoupon(community.id, code);
      if (res.ok) {
  ...
  async function buy() {
    ...
    setStatus("starting");
    try {
      const result = await startCommunityCheckout(community.id, { email, contact }, applied?.code);
      if (!result.ok) {
  ...
  if (status === "paid") {
    return (
      <PaymentSuccess
        title="You’re in!"
        subtitle="Payment successful — your community is ready."
        ctaHref={`/account/community/${community.slug}`}
  ...
  // FREE community — a one-tap join (sign-in required, handled in the action).
  if (isFree) {
    return (
      <div className="mt-5">
        {error ? (
        ...
        >
          {status === "starting" ? "Joining…" : "Join for free"}
        </button>
        <p className="mt-2 text-center text-xs text-muted">Sign in to join — it’s free.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-5">
      ...
      {method === "razorpay" && razorpayReady ? (
        ...
        >
          {status === "starting" ? "Starting…" : `Join · ${formatRupees(total)}`}
        </button>
      ) : null}
      {method === "upi" && upi ? (
        <UpiPayPanel
          ...
          onStart={() => startCommunityUpiSession(community.id, { email, contact }, applied?.code)}
          onConfirmed={() => setStatus("paid")}
          onSubmitted={() => setUpiDone(true)}
        />
      ) : null}
      <p className="mt-2 text-center text-xs text-muted">
        Use this email to sign in and access the community after paying.
      </p>
    </div>
  );
}
