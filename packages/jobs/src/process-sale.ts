...
/** The data needed to run all sale-time notifications for one newly-PAID order.
 *  This is the queue job payload AND the inline-call argument — identical either
 *  way, so behaviour is the same whether processed by the worker or inline. */
 ...
 *
 * Moved out of apps/tenant so the standalone BullMQ worker can run the exact same
 * logic; the inline (queue-disabled) path calls it directly too.
 */
export async function processSaleNotification(input: SaleNotificationPayload): Promise<void> {
  try {
